...
const WavingScene = dynamic(
  () => import("../components/WavingScene").then((mod) => mod.WavingScene),
  {
    ssr: false,
    loading: () => (
...
export default function WavingPage() {
    ...
    <div className="relative min-h-screen w-full overflow-hidden bg-white">
      ...
      <div className="fixed right-6 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-2 rounded-full border border-neutral-200 bg-white/90 px-3 py-4 shadow-lg backdrop-blur-sm">
        {colors.map((c) => (
          <button
            ...
            className={`h-7 w-7 rounded-full border-2 transition-all hover:scale-110 ${color === c.value ? "border-black scale-110" : "border-transparent"
              } ${c.bg}`}
            title={c.name}
          />
        ))}
      ...
      <main className="relative z-10 flex min-h-screen flex-col items-center justify-center px-4">
        ...
        <div className="relative h-[55vh] md:h-[60vh] w-full max-w-3xl">
          ...
          {/* 3D Scene */}
          <WavingScene personColor={color} />
        </div>

        {/* Stats Section */}
        <div className="mt-4 flex flex-wrap justify-center gap-12 md:gap-20 opacity-50">
          <StatCard value="👋" label="Waves" />
          <StatCard value="100%" label="Friendly" />
        </div>
      </main>
